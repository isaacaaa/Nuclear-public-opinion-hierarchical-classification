    ...
    Returns:
        ...
        if labels is not None:
            if self.num_labels == 1:
                ...
                stance_weight=[1/1734,1/1613,1/2095]
                stance_weight=torch.FloatTensor(stance_weight).cuda()
                loss_fct_for_stance=CrossEntropyLoss(weight=stance_weight)

                loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1))
                outputs = (loss,) + outputs
                loss1=loss_fct1(logits1.view(-1,self.num_labels1),labels1.view(-1
))
                logits_list=torch.argmax(logits,dim=1).tolist()
                logits1_list=torch.argmax(logits1,dim=1).tolist()
                for index ,i in enumerate(logits_list):
                    for index1 ,j in enumerate(logits1_list):
                        if i ==1 and j==0 or i==1 and j==1 or i==1 and j==2:
                            loss1[index1]=loss1[index1]*1.5
                        if i==0 and j==3:
                            loss1[index1]==loss1[index1]*1.5
        ...
        return outputs,outputs1,outputs2  # (loss), logits, (hidden_states), (attentions)
